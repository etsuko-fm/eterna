...
local FilterLfoGraphic = include(from_root("lib/graphics/FilterLfoGraphic"))
local lfo
...
local function create_filter_page(config)
    ...
    local function action_ctrl_mode(v)
        ...
        print('mode: ' .. params:string(ID_CTRL_MODE))
        print('wet: '.. params:get(ID_WET))
    ...
    function page:update_graphics_state()
        ...
        self.filter_graphic:set("freq", base_freq)
        self.filter_graphic:set("lfo_freq", freq)
        self.filter_graphic:set("res", res)
        self.filter_graphic:set("type", FILTER_TYPE)

        -- mix can be 1 (0%), 2 (50%) or 3 (100%)
        self.filter_graphic:set("mix", (mix - 1) / 2)
        local route_txt = ""
        ...
        -- render non-modulated frequency
        self.filter_graphic:set("freq", freq)
        self.filter_graphic:set_lfo_range(low, high)
        self.filter_graphic:set("res", res)
        self.filter_graphic:set("type", FILTER_TYPE)
        self.filter_graphic:set("mix", (mix - 1) / 2)
        self.filter_graphic:set("rate_fraction", params:get(ID_LFO_RATE) / #lfo_util.lfo_period_labels)
        self.filter_graphic:set("draw_lfo_range", mix > 1)

        -- self.graphic = control_mode == CTRL_MODE_LFO and self.lfo_graphic or self.filter_graphic
    end
    ...
    function page:initialize()
        ...
        self.filter_graphic = FilterGraphic:new()
        self.filter_graphic:set_size(62, 27)
        self.filter_graphic:set("draw_lfo_range", false)
        self.filter_graphic:set("lfo_freq", params:get(ID_BASE_FREQ))

        self.lfo_graphic = FilterLfoGraphic:new()
        self.lfo_graphic:set('current', 123)

        self.graphic = self.filter_graphic
        ...
        lfo = _lfos:add({
            ...
            action = function(scaled, raw)
                ...
                page.lfo_graphic:set('current', raw)
            end
        })
