  ...
  alloc {
    ...
    this.addCommand("set_buffer","si", { 
      ...
      r = Routine {
        ...
        buffL = Buffer.readChannel(context.server, msg[1].asString, channels:[0], bufnum: bufnumL, action: {|b| ("Channel 1 loaded to buffer " ++ bufnumL).postln;});
        if (f.numChannels > 1) {
          // It may be quadraphonic or surround, but that's not supported right now
          "Loading channel 2".postln;
          buffR = Buffer.readChannel(context.server, msg[1].asString, channels:[1], bufnum: bufnumR, action: {|b| ("Channel 2 loaded to buffer " ++ bufnumR).postln;});
        };
        context.server.sync();
        isLoaded = true;
